...
const player = world.spawn(Position({ x: 100, y: 0, z: 50 }), Velocity, Health, IsPlayer)
```
...
world.spawn(DamageDealtEvent({ target: targetEntity, amount: dmg, source: attacker }))
```
...
```ts
// actions/combat.ts
export function dealDamage(attacker: Entity, target: Entity, baseDamage: number) {
  const armor = target.get(Defense)?.armor ?? 0
  const finalDamage = calculateDamage(baseDamage, armor)

  target.set(Health, (h) => ({ current: Math.max(0, h.current - finalDamage) }))
...
```ts
// Spawning
const enemy = world.spawn(Position({ x: 10, y: 0, z: 20 }), Health, IsEnemy, AIController)
...
const DamageDealtEvent  = trait({ target: 0 as Entity, source: 0 as Entity, amount: 0, kind: 'melee' as 'melee' | 'ranged' | 'fall' })
const EntityDiedEvent   = trait({ entity: 0 as Entity, cause: 'combat' as 'combat' | 'fall' | 'starvation' })
const SoundRequested    = trait({ soundId: '' as SoundId, x: 0, y: 0, z: 0, volume: 1 })
...
const GameEvent = trait({ type: '' as string, data: {} as Record<string, unknown> })
```

**Spawning an event is the emit:**

```ts
world.spawn(SoundRequested({ soundId: 'sword_hit', x: pos.x, y: pos.y, z: pos.z, volume: 0.8 }))
```
...
export function gameplayEventPhase(world: World) {
  world.query(DamageDealtEvent).updateEach(([ev], entity) => {
    const hp = ev.target.get(Health); if (!hp) return entity.destroy()
    ev.target.set(Health, { current: Math.max(0, hp.current - ev.amount) })
    if (ev.target.get(Health)!.current <= 0) world.spawn(EntityDiedEvent({ entity: ev.target, cause: 'combat' }))
    entity.destroy()
  })

  world.query(EntityDiedEvent).updateEach(([ev], entity) => {
    ev.entity.add(IsDead)
    world.spawn(SoundRequested({ soundId: 'death', x: 0, y: 0, z: 0, volume: 1 }))
    entity.destroy()
  })
}
```
...
export function combatSystem(world: World, delta: number) {
  world.query(AttackState, Position).updateEach(([attack, pos]) => {
    ...
    world.spawn(SoundRequested({ soundId: 'sword_hit', x: pos.x, y: pos.y, z: pos.z, volume: 1 }))
    world.spawn(VfxRequested({   vfxId:   'hit_spark', x: pos.x, y: pos.y, z: pos.z }))
    attack.didHit = false
  })
}
...
export function audioBridgePhase(world: World) {
  world.query(SoundRequested).updateEach(([req], entity) => {
    howlerEngine.play(req.soundId, { x: req.x, y: req.y, z: req.z, volume: req.volume })
    entity.destroy()
  })
}
```
...
const FloatingText = trait({ active: false, text: '', x: 0, y: 0, z: 0, progress: 0 })
...
export function spawnFloatingText(world: World, text: string, x: number, y: number, z: number) {
...
## Rule 24: Render with WebGPU + TSL — No Raw GLSL ShaderMaterial

**The problem:** Mixing rendering backends and shader languages fragments the codebase. Raw GLSL `ShaderMaterial` and `onBeforeCompile` string patching do not run under a true WebGPU backend, so any GLSL shader silently breaks (or forces a WebGL-only fallback) the moment we rely on WebGPU features.

**The rule:** This project renders with three.js **`WebGPURenderer`** (from `three/webgpu`). Author all custom shading in **TSL** (Three Shading Language, `three/tsl`) via node materials — never raw GLSL. `WebGPURenderer` auto-falls back to a WebGL2 backend where WebGPU is unavailable, so the same TSL code runs everywhere (keeps Poki/mobile coverage). Core materials (`MeshStandardMaterial`, `LineBasicMaterial`, …) are auto-converted and remain fine; the ban is specifically on hand-written GLSL.

```ts
// providers.tsx — the renderer is created once, here (Rule 22)
import * as THREE from 'three/webgpu'
import { Canvas, extend } from '@react-three/fiber'

extend(THREE as any) // register node materials as JSX elements (boundary `any`)
...
```ts
// BAD — raw GLSL, does not run on the WebGPU backend
const mat = new THREE.ShaderMaterial({ vertexShader, fragmentShader })
standardMat.onBeforeCompile = (s) => { s.fragmentShader = s.fragmentShader.replace(/* GLSL string patching */) }

// GOOD — TSL node material, portable across WebGPU/WebGL2
import { MeshStandardNodeMaterial } from 'three/webgpu'
import { texture, uv, mix, triplanarTexture } from 'three/tsl'
const mat = new MeshStandardNodeMaterial()
mat.colorNode = mix(baseColorNode, sceneTextureNode, blendNode)
```

Use the `webgpu-threejs-tsl` skill (`.agents/skills/webgpu-threejs-tsl`) when writing TSL. Note: drei components backed by raw `ShaderMaterial` (e.g. `<Grid>`) are off-limits — prefer core helpers (`<gridHelper>`) or a TSL equivalent. This rule pairs with Rule 17 (pooled VFX) and is the foundation the paint/disguise shaders build on.

---

## Rule 25: No Comments — The Code Explains Itself

**The problem:** Comments rot. They drift out of sync with the code, restate what a well-named symbol already says, or paper over logic that is hard to read. A comment is almost always a signal that the code failed to communicate on its own — the fix is to change the code, not to annotate it.

**The rule:** Do not write comments. No line comments, no block comments, no JSDoc, no "section header" banners, no commented-out code. Make the code self-explanatory instead: extract a named function (Rule 19), name every data shape (Rule 21), pull magic numbers into `GAME_CONFIG` (Rule 16), and use domain-specific, question-shaped names (Extra Rules 6–7). If you feel a comment is necessary, treat it as a code smell: something is unclear enough that it should be refactored until it reads without explanation.

```ts
// BAD — a comment compensating for unclear code
// clamp so the value stays within the allowed brush range
const r = Math.min(Math.max(v, 0.05), 2.0)

// eat the buffered delta so a stale move can't snap the camera later
d.x = 0
d.y = 0

// GOOD — the names and structure carry the meaning; no comment needed
const radius = clamp(v, GAME_CONFIG.PAINT.BRUSH_RADIUS_MIN, GAME_CONFIG.PAINT.BRUSH_RADIUS_MAX)

resetBufferedLook(delta)
```

When you reach for a comment, reach for one of these instead: a `const` with a descriptive name, a small pure function whose name is the sentence you were about to write, or a named type. The only text that ships is code that says what it means. See Rule 19 (reusable functions), Rule 21 (named shapes), and Rule 16 (named constants).
